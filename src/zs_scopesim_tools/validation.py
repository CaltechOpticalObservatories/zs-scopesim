...
from .plots import (plot_detector_background_budget, plot_detector_image_grid, plot_emissivity_sanity,
                    plot_post_disperser_diffuse_background, plot_readout_delta_overview,
                    plot_readout_cross_dispersion_cut, plot_readout_overview, plot_slit_adc_psf_scenes,
                    plot_slit_loss_by_arm, plot_slit_pair_geometry, plot_slit_width_loss, plot_source,
                    plot_resolving_power_echellogram, plot_transmission_sanity)
...
def get_effect(ztrain: Any, display_name: str, *, active_only: bool = True) -> Any:
    ...
    if len(matches) != 1:
        ...
        raise ValueError(f"Expected one active effect named {display_name!r}; found " f"{len(matches)}. Available: {available}")
    return matches[0]
...
def resolve_effect(effect: Any, selector_value: Any = None) -> Any:
    ...
    try:
        ...
        keys = sorted(effect.wheel_effects)
        raise KeyError(f"{effect_name(effect)!r} has no selector value " f"{selector_value!r}; available {keys}") from exc
...
def _evaluate_diffuse_throughput(effect: Any, wave: u.Quantity, *, footprint: Any = None) -> np.ndarray:
    if hasattr(effect, "effective_diffuse_throughput"):
        with u.set_enabled_equivalencies(u.spectral()):
            values = effect.effective_diffuse_throughput(wave, footprint=footprint)
...
def _looks_like_qe_effect(effect: Any) -> bool:
    ...
    if "qe" in name or "quantumefficiency" in class_name:
        return True
    return any("quantumefficiency" in candidate.__class__.__name__.lower() for candidate in _candidate_qe_effects(effect))


def _get_qe_selector(ztrain: Any, qe_selector_name: str | None, *, active_only: bool = True) -> Any:
    if qe_selector_name is not None:
        return get_effect(ztrain, qe_selector_name, active_only=active_only)

    effects = active_effects(ztrain) if active_only else ztrain.optics_manager.all_effects
    candidates = [effect for effect in effects if _looks_like_qe_effect(effect)]
    if len(candidates) != 1:
        available = [effect_name(effect) for effect in candidates] or sorted(effect_name(effect) for effect in effects)
        raise ValueError("Expected exactly one enabled QE selector when " "qe_selector_name=None; found " f"{len(candidates)}. Pass qe_selector_name explicitly. " f"Candidates/available effects: {available}")
    return candidates[0]
...
def _is_surface_effect(effect: Any) -> bool:
    surface = getattr(effect, "surface", None)
    return surface is not None and any(hasattr(surface, attr) for attr in ("throughput", "transmission", "reflection", "emissivity"))


def _surface_effect_group_name(parent: Any, effect: Any, component_metadata: Mapping[str, Any] | None = None) -> str:
    ...
    override_group = _clean_metadata_value((component_metadata or {}).get("throughput_group"))
    if configured_group is not None and override_group is not None:
        if configured_group != override_group:
            raise ValueError(f"{effect_name(parent)!r} configured throughput_group " f"{configured_group!r} conflicts with explicit override " f"{override_group!r}.")
    if configured_group is not None:
        return configured_group
    if override_group is not None:
        return override_group

    raise ValueError(f"{effect_name(parent)!r} selected {effect_name(effect)!r} without " "throughput_group metadata.")


def _surface_effect_phase_name(parent: Any, effect: Any, component_metadata: Mapping[str, Any] | None = None) -> str:
    ...
    for meta in (getattr(effect, "meta", {}), getattr(parent, "meta", {})):
        ...
        if phase_name is not None:
            ...
            break
    override_phase = _clean_emission_phase((component_metadata or {}).get("emission_phase"))
    if configured_phase is not None and override_phase is not None:
        if configured_phase != override_phase:
            raise ValueError(f"{effect_name(parent)!r} configured emission_phase " f"{configured_phase!r} conflicts with explicit override " f"{override_phase!r}.")
    if configured_phase is not None:
        return configured_phase
    if override_phase is not None:
        return override_phase
    raise ValueError(f"{effect_name(parent)!r} selected {effect_name(effect)!r} without " "emission_phase metadata.")
...
def channel_optical_components(ztrain: Any, aperture_id: int, *, qe_selector: Any | None = None) -> list[dict[str, Any]]:
...
def optical_surface_rows(components: list[dict[str, Any]], wave: u.Quantity, component_metadata: Mapping[str, Mapping[str, Any]] | None = None) -> list[dict[str, Any]]:
...
def fetch_effect_spectrum_or_transmission(ztrain: Any, display_name: str, attribute: str | None = "throughput", wave: u.Quantity | None = None, aperture_id: int | None = None) -> tuple[u.Quantity, Any]:
...
def surface_group_for_row(row: Any) -> str:
    ...
    if group_col is None:
        raise ValueError(f"Surface row {surface_name!r} has no throughput_group column.")
    group_name = _clean_metadata_value(_row_scalar(row, group_col))
    if group_name is None:
        raise ValueError(f"Surface row {surface_name!r} has an empty throughput_group.")
    return group_name
...
def emission_phase_for_row(row: Any) -> str:
    ...
    if phase_col is None:
        raise ValueError(f"Surface row {surface_name!r} has no emission_phase column.")
    phase_name = _clean_emission_phase(_row_scalar(row, phase_col))
    if phase_name is None:
        raise ValueError(f"Surface row {surface_name!r} has an empty emission_phase.")
    return phase_name
...
def ter_property_sources(surface: Any) -> dict[str, str]:
    ...
    explicit = {name: name in surface.meta or name in colnames for name in ("transmission", "reflection", "emissivity")}
    sources = {}
    for name, is_explicit in explicit.items():
        if is_explicit:
            sources[name] = "explicit"
        else:
            other_names = [other for other, exists in explicit.items() if other != name and exists]
            if other_names:
...
def evaluate_ter_property(effect_or_surface: Any, property_name: str, wave: u.Quantity) -> np.ndarray:
...
def optical_surface_group_throughputs(rows: list[dict[str, Any]], wave: u.Quantity) -> tuple[OrderedDict[str, np.ndarray], dict[str, int]]:
...
def optical_surface_emissivity_terms(rows: list[dict[str, Any]], wave: u.Quantity, qe_values: np.ndarray | None = None) -> tuple[dict[str, OrderedDict[str, np.ndarray]], dict[str, int], list[dict[str, Any]]]:
    ...
    for idx, row in enumerate(rows):
        ...
        if phase_name not in phase_terms:
            raise ValueError(f"Unknown emission_phase {phase_name!r} for " f"{row['surface_name']!r}; expected one of " f"{sorted(phase_terms)} or 'none'.")
        group_name = row["group"]
...
def effective_diffuse_qe(detector_qe: Any, wave: u.Quantity, positional_qe: Callable[[u.Quantity], Any] | np.ndarray | None = None, footprint: Any = None) -> np.ndarray:
    ...
    """
    spectral_qe = _evaluate_diffuse_throughput(detector_qe, wave, footprint=footprint)
...
def detector_qe_accounting_summary(detector_qe: Any) -> dict[str, Any]:
    ...
    return {
        **qe_detail,
        "trace_mapped_qe": (
            "per-pixel detector coordinates from trace mapping" if is_position_aware else "wavelength-only spectral throughput"
        ),
        "transmission_plot_qe": (
            "footprint/detector-axis average shown for display; order totals " "use slit-center trace detector coordinates" if uses_footprint else "same wavelength-only spectral throughput used for all orders"
        ),
        "diffuse_qe": (
            "mean over uniformly sampled detector-axis positions" if uses_footprint else "wavelength-only spectral throughput"
        ),
...
def _trace_center_detector_positions(trace: Any, wave: u.Quantity, image_plane: Any | None = None) -> dict[str, np.ndarray] | None:
    """Return slit-center detector coordinates for a trace and wave grid."""
    coords = _trace_detector_positions(trace, wave, image_plane, slit_positions_arcsec=np.array([0.0]))
    if coords is None:
        return None
    return {key: np.asarray(value)[0] for key, value in coords.items()}
...
def _trace_detector_positions(trace: Any, wave: u.Quantity, image_plane: Any | None = None, *, slit_positions_arcsec: np.ndarray | None = None) -> dict[str, np.ndarray] | None:
    ...
    try:
        ...
        x_pix, y_pix = WCS(image_plane.header, key="D").all_world2pix(x_mm.ravel(), y_mm.ravel(), 0)
...
def evaluate_trace_detector_qe(detector_qe: Any, trace: Any, wave: u.Quantity, image_plane: Any | None = None) -> tuple[np.ndarray, str]:
    ...
    if coords is None:
        return (evaluate_throughput(detector_qe, wave), "representative detector midpoint")

    with u.set_enabled_equivalencies(u.spectral()):
        values = detector_qe.throughput_at(wave, detector_x=coords.get("detector_x"), detector_y=coords.get("detector_y"), detector_x_mm=coords["detector_x_mm"], detector_y_mm=coords["detector_y_mm"], trace=trace)
    method = "slit-center trace detector pixels" if "detector_y" in coords or "detector_x" in coords else "representative detector midpoint; detector-pixel WCS unavailable"
...
def trace_resolution_diagnostic_table(ztrain: Any, *, trace_list_name: str = "trace_list_analytical", allow_diagnostic_psf: bool = False) -> Table:
    ...
    for aperture_id, traces in traces_by_aperture(trace_list).items():
        ...
        for trace in traces:
            ...
            if np.any(np.diff(node_wave) <= 0 * u.um) or np.any(node_dx_pix <= 0) or np.any(np.abs(node_dx_pix) <= np.abs(node_dy_pix)) or not (np.all(np.isfinite(node_x_pix)) and np.all(np.isfinite(node_y_pix))):
                raise NotImplementedError(f"Trace {trace_id!r} does not have one continuous, " f"wavelength-ordered detector-x run.")
            ...
            domain_coordinates = ztrain.trace_detector_coordinates(xi=0 * u.arcsec, wavelengths={trace_id: u.Quantity([domain_wave_low, domain_wave_high])})
            ...
            complete = (detector_x0_pix >= 0) & (detector_x1_pix <= detector_naxis1) & (detector_x1_pix > detector_x0_pix) & (raster_x_low >= domain_x_low) & (raster_x_high <= domain_x_high)
            ...
            center_coordinates = ztrain.trace_detector_coordinates(xi=0 * u.arcsec, wavelengths={trace_id: requested_wave_nm * u.nm})
            ...
            if len(center_coordinates) != len(requested_wave_nm) or np.any(center_trace_ids != trace_id) or not (np.all(np.isfinite(detector_x_pix)) and np.all(np.isfinite(detector_y_pix))) or np.any(detector_x_pix < raster_x_low) or np.any(detector_x_pix > raster_x_high):
...
def trace_resolution_summary_table(table: Table) -> Table:
    ...
    for key in table.group_by(["image_plane_id", "channel"]).groups.keys:
        ...
        group = table[(table["channel"] == channel) & (table["image_plane_id"] == image_plane_id)]
        ...
        rows.append({
            ...
            "seeing_fwhm_median_arcsec": np.median(group["seeing_fwhm_arcsec"]),
...
def _compact_range(minimum: Any, maximum: Any, *, precision: int, scale: float = 1.0) -> str:
...
def trace_resolution_summary_display_table(table: Table):
    ...
    for row in table:
        rows.append({
            ...
            "disp nm/pix": _compact_range(row["dispersion_min_nm_pix"], row["dispersion_max_nm_pix"], precision=4),
            "R k": _compact_range(row["trace_R_min"], row["trace_R_max"], precision=1, scale=1000.0),
            "resel pix": _compact_range(row["resel_footprint_min_pix"], row["resel_footprint_max_pix"], precision=1),
            "seeing": (
...
def fov_image_plane_counts(ztrain: Any) -> Table:
    ...
    image_plane_ids = [int(fov.meta["image_plane_id"]) for fov in ztrain.fov_manager.fovs]
...
def build_emissivity_sanity_data(
    ztrain: Any, wave_nm: u.Quantity | None = None, component_metadata: Mapping[str, Mapping[str, Any]] | None = None, positional_qe_by_aperture: Mapping[int, Any] | None = None,
    qe_selector_name: str | None = "detector_qe_selector", active_only: bool = True, blocking_component_names: tuple[str, ...] = ("ir_blocking_filter_selector",), diffuse_extraction_pixels: float = 1.0,
) -> dict[str, Any]:
    ...
    for aperture_id in aperture_ids:
        ...
        surface_rows = optical_surface_rows(components, wave, component_metadata=component_metadata)
        ...
        unblocked_phase_terms, _unblocked_counts, _unblocked_details = optical_surface_emissivity_terms(unblocked_rows, wave, qe_values=qe_values)
        ...
        diffuse_rate_per_pix = integrate_spectral_background(post_total * PHOTLAM, wave, telescope_area=telescope_area, image_pixel_area=image_pixel_area)
        diffuse_unblocked_rate_per_pix = integrate_spectral_background(post_unblocked_total * PHOTLAM, wave, telescope_area=telescope_area, image_pixel_area=image_pixel_area)

        diffuse_extract_equiv = np.full(wave.size, diffuse_rate_per_pix * diffuse_extraction_pixels, dtype=float)
        diffuse_unblocked_extract_equiv = np.full(wave.size, diffuse_unblocked_rate_per_pix * diffuse_extraction_pixels, dtype=float)
...
def optical_surface_post_disperser_diffuse_terms(rows: list[dict[str, Any]], wave: u.Quantity, qe_values: np.ndarray | None = None, emission_phase: str = "post_disperser") -> tuple[OrderedDict[str, u.Quantity], list[dict[str, Any]]]:
    """Return post-disperser diffuse spectra from flattened surface rows."""
    for row in rows:
        phase_name = row["emission_phase"]
        if phase_name not in {"pre_disperser", "post_disperser", "none"}:
            raise ValueError(f"Unknown emission_phase {phase_name!r} for " f"{row['surface_name']!r}; expected pre_disperser, " "post_disperser, or none.")
    ...
    for idx, row in enumerate(rows):
        ...
        if is_diffuse and row["emission_values"] is not None:
            ...
            group_name = row["group"]
            grouped[group_name] = contribution if group_name not in grouped else grouped[group_name] + contribution
...
def _image_plane_pixel_area(ztrain: Any, image_plane_id: int) -> u.Quantity:
    ...
    return image_plane_pixel_area(ztrain.image_planes[image_plane_id].header, ztrain.cmds)
...
def build_post_disperser_diffuse_background_data(
    ztrain: Any, wave_nm: u.Quantity | None = None, component_metadata: Mapping[str, Mapping[str, Any]] | None = None, positional_qe_by_aperture: Mapping[int, Any] | None = None,
    qe_selector_name: str | None = "detector_qe_selector", active_only: bool = True, blocking_component_names: tuple[str, ...] = ("ir_blocking_filter_selector",),
) -> dict[str, Any]:
    ...
    qe_selector = _get_qe_selector(ztrain, qe_selector_name, active_only=active_only)
    trace_list = get_effect(ztrain, "trace_list_analytical")
    post_diffuse_selector = _optional_effect(ztrain, "post_echelle_diffuse_background_selector", active_only=active_only)
    traces_for_aperture = traces_by_aperture(trace_list)
    ...
    for aperture_id in aperture_ids:
        ...
        image_plane_id = int(traces[0].meta["image_plane_id"]) if traces else aperture_id
        image_pixel_area = _image_plane_pixel_area(ztrain, image_plane_id)
        image_plane = ztrain.image_planes[image_plane_id] if image_plane_id < len(ztrain.image_planes) else None
        effect_data = _post_disperser_diffuse_data_from_effect(post_diffuse_selector, image_plane_id, image_plane, wave, component_metadata=component_metadata)
        if effect_data is None:
            qe_values = effective_diffuse_qe(detector_qe, wave, positional_qe=positional_qe_by_aperture.get(aperture_id), footprint=image_plane)
            components = channel_optical_components(ztrain, aperture_id, qe_selector=qe_selector)
            surface_rows = optical_surface_rows(components, wave, component_metadata=component_metadata)
            spectra, surface_details = optical_surface_post_disperser_diffuse_terms(surface_rows, wave, qe_values=qe_values)
            unblocked_rows = _rows_excluding_components(surface_rows, blocked_components)
            spectra_unblocked, _surface_details_unblocked = optical_surface_post_disperser_diffuse_terms(unblocked_rows, wave, qe_values=qe_values)
            detector_qe_accounting = detector_qe_accounting_summary(detector_qe)
        else:
        ...
        rates = OrderedDict((name, integrate_spectral_background(spectrum, wave, telescope_area=telescope_area, image_pixel_area=image_pixel_area)) for name, spectrum in spectra.items())
        rates_unblocked = OrderedDict((name, integrate_spectral_background(spectrum, wave, telescope_area=telescope_area, image_pixel_area=image_pixel_area)) for name, spectrum in spectra_unblocked.items())
        ...
        total_rate_unblocked = float(np.sum(list(rates_unblocked.values()))) if rates_unblocked else 0.0
        label = channel_label(aperture_id, traces)
        ...
        channels[aperture_id] = {
            ...
            "image_plane_id": image_plane_id,
            "trace_wave_min_nm": (
                min(_trace_wave_nm(trace.wave_min) for trace in traces) if traces else np.nan
            ),
            "trace_wave_max_nm": (
                max(_trace_wave_nm(trace.wave_max) for trace in traces) if traces else np.nan
            ),
            "spectra": spectra,
...
def _optional_effect(ztrain: Any, display_name: str, *, active_only: bool = True) -> Any | None:
    effects = active_effects(ztrain) if active_only else ztrain.optics_manager.all_effects
    matches = [eff for eff in effects if effect_name(eff) == display_name]
    if len(matches) > 1:
        raise ValueError(f"Expected at most one active effect named {display_name!r}; found " f"{len(matches)}.")
    return matches[0] if matches else None


def _post_disperser_diffuse_data_from_effect(selector: Any | None, image_plane_id: int, image_plane: Any, wave: u.Quantity, *, component_metadata: Mapping[str, Mapping[str, Any]] | None = None) -> dict[str, Any] | None:
    ...
    if not all(hasattr(effect, attr) for attr in ("_surface_list", "_detector_qe", "_downstream_throughput_values")):
        return None

    qe_values = effective_diffuse_qe(effect._detector_qe, wave, positional_qe=getattr(effect, "_positional_qe", None), footprint=image_plane)
    surface_rows = optical_surface_rows([{
        "selector": selector,
        "selector_name": effect_name(selector),
        "effect": effect._surface_list,
    }], wave, component_metadata=component_metadata)
    spectra_unblocked, surface_details = optical_surface_post_disperser_diffuse_terms(surface_rows, wave, qe_values=qe_values, emission_phase=effect.meta.get("emission_phase", "post_disperser"))
    downstream = effect._downstream_throughput_values(wave)
    spectra = OrderedDict((name, spectrum * downstream) for name, spectrum in spectra_unblocked.items())
    for row in surface_details:
    ...
    return {
        ...
        "detector_qe": qe_values,
        "detector_qe_accounting": detector_qe_accounting_summary(effect._detector_qe),
    }


def detector_qe_accounting_table(transmission_data: Mapping[str, Any] | None = None, emissivity_data: Mapping[str, Any] | None = None, post_diffuse_data: Mapping[str, Any] | None = None) -> Table:
    ...
    for path_name, data in datasets:
        ...
        for aperture_id, channel in data["channels"].items():
            ...
            rows.append({
                ...
                "diffuse_position_samples": accounting.get("diffuse_position_samples"),
                "order_qe_methods": ", ".join(channel.get("order_detector_qe_methods", [])),
...
def post_disperser_diffuse_effect_consistency_table(
    ztrain: Any, helper_data: Mapping[str, Any], *, effect_display_name: str = "post_echelle_diffuse_background_selector", match_effect_grid: bool = True,
    component_metadata: Mapping[str, Mapping[str, Any]] | None = None, qe_selector_name: str | None = "detector_qe_selector", active_only: bool = True,
) -> Table:
    ...
    if match_effect_grid:
        matched_data_by_image_plane = _matched_post_diffuse_data_by_image_plane(ztrain, helper_data, selector, qe_selector_name=qe_selector_name, component_metadata=component_metadata, active_only=active_only)
    ...
    for aperture_id, channel in helper_data["channels"].items():
        ...
        matched_channel = matched_data_by_image_plane.get(image_plane_id, {}).get("channels", {}).get(aperture_id)
        if match_effect_grid and matched_channel is None:
            raise ValueError(f"No matched helper data for image_plane_id={image_plane_id}, " f"aperture_id={aperture_id}.")
        matched_rate = float(matched_channel["total_rate_ph_s_pix"]) if (matched_channel is not None) else np.nan
        rows.append({
            "aperture_id": int(aperture_id),
            "channel": channel["label"],
            "image_plane_id": image_plane_id,
...
def validate_post_disperser_diffuse_effect_consistency(table: Table, *, rtol: float = 1e-6, use_matched: bool = True) -> None:
    ...
    if np.nanmax(abs_delta) > rtol:
        ...
        raise ValueError(f"Post-disperser diffuse helper/effect mismatch above {rtol}: {bad}")


def _matched_post_diffuse_data_by_image_plane(ztrain: Any, helper_data: Mapping[str, Any], selector: Any, *, qe_selector_name: str | None = "detector_qe_selector", component_metadata: Mapping[str, Mapping[str, Any]] | None = None, active_only: bool = True) -> dict[int, Mapping[str, Any]]:
    matched = {}
    cache = {}
    for channel in helper_data["channels"].values():
        ...
        if not hasattr(effect, "_waveset"):
            raise ValueError(f"{effect_name(effect)!r} for image_plane_id={image_plane_id} " "has no _waveset method for matched-grid validation.")
        wave_nm = effect._waveset().to(u.nm)
        cache_key = tuple(np.round(wave_nm.to_value(u.nm), 12))
        if cache_key not in cache:
            cache[cache_key] = build_post_disperser_diffuse_background_data(ztrain, wave_nm=wave_nm, component_metadata=component_metadata, qe_selector_name=qe_selector_name, active_only=active_only)
...
def _rows_excluding_components(rows: list[dict[str, Any]], excluded_components: set[str]) -> list[dict[str, Any]]:
    if not excluded_components:
        return rows
    return [row for row in rows if str(row["component"]) not in excluded_components]
...
def validate_post_disperser_diffuse_background_data(data: Mapping[str, Any]) -> None:
    ...
    for aperture_id, channel in data["channels"].items():
        if not channel["spectra"]:
            raise ValueError(f"No post-disperser diffuse spectra for aperture_id={aperture_id}")
        if channel["total_rate_ph_s_pix"] < -1e-12:
            raise ValueError(f"Negative diffuse background for aperture_id={aperture_id}: " f"{channel['total_rate_ph_s_pix']}")
...
def source_position_table(source_or_table: Any) -> Table:
    ...
    if isinstance(source_or_table, Table):
        ...
        table_fields = [field.field for field in source_or_table.fields if hasattr(field, "field") and isinstance(field.field, Table)]
        if len(table_fields) != 1:
            raise ValueError("Expected exactly one table source field; found " f"{len(table_fields)}.")
...
def slit_pair_status_table(source_or_table: Any, *, slit_width: u.Quantity = 0.7 * u.arcsec, slit_length: u.Quantity = 10.0 * u.arcsec) -> Table:
    ...
    labels = [str(value) for value in table["label"]] if "label" in table.colnames else [f"source_{idx}" for idx in range(len(table))]
    rows = []
...
def slit_adc_psf_status_table(ztrain: Any, *, slit_selector_name: str = "slitwheel_selector") -> Table:
    ...
    rows.extend(_keyword_effect_rows(ztrain, category="atmospheric dispersion", keywords=("adcshift", "atmosphericdispersion", "dispersioncorrection")))
    rows.extend(_keyword_effect_rows(ztrain, category="psf", keywords=("psf", "seeing", "moffat", "diffraction", "aoenhanceable")))
...
def _slit_setting_rows(ztrain: Any, selector_name: str) -> list[dict[str, Any]]:
    ...
    for selector_value, effect in sorted(selector.wheel_effects.items()):
        current_slit = _resolved_for_display(getattr(effect, "meta", {}).get("current_slit", ""), ztrain.cmds)
    ...
    for (class_name, current_slit, filename_format), selector_values in groups.items():
        rows.append({
            "category": "slit",
            "scope": _selector_scope(selector.meta.get("selector_key", "selector"), selector_values),
            "name": selector_name,
            "class": class_name,
...
def _slit_selector_widths_for_key(ztrain: Any, slit_key: str, *, selector_name: str = "slitwheel_selector") -> u.Quantity:
...
def _keyword_effect_rows(ztrain: Any, *, category: str, keywords: tuple[str, ...]) -> list[dict[str, Any]]:
    rows = []
...
def _matching_surface_names(effect: Any, patterns: tuple[str, ...]) -> list[str]:
    ...
    return [str(name) for name in table[name_col] if any(pattern in str(name).upper() for pattern in pattern_upper)]
...
def _required_cmd_float(cmds: Any, key: str) -> float:
    ...
    try:
        ...
        raise ValueError(f"Required ScopeSim command {key!r} must resolve to a float; " f"got {value!r}.") from exc
...
def _natural_seeing_fwhm(wave: u.Quantity, seeing: u.Quantity, zenith_angle: u.Quantity, *, pivot: u.Quantity = 500 * u.nm) -> u.Quantity:
...
def _atmospheric_refraction_shift(wave: u.Quantity, zenith_angle: u.Quantity, cmds: Any, *, wave_ref: u.Quantity = 500 * u.nm) -> u.Quantity:
    ...
    delta_n = refractive_index(wave_um, temperature, pressure, humidity, x_co2) - refractive_index(ref_um, temperature, pressure, humidity, x_co2)
...
def _adc_zenith_angle_error(train_or_cmds: Any, default: u.Quantity) -> tuple[u.Quantity, str]:
    ...
    for effect in active_effects(train_or_cmds):
        ...
        if value is None:
            continue
        return u.Quantity(float(_resolved_for_display(value, train_or_cmds.cmds)), u.deg), (f"active {effect_name(effect)} zenith-angle residual")

    return u.Quantity(default).to(u.deg), ("diagnostic ADC residual; no active ADCShift effect found")


def _moffat_image(x_grid: np.ndarray, y_grid: np.ndarray, *, x0: float, y0: float, fwhm: float, beta: float) -> np.ndarray:
...
def _scene_image(positions: Table, wave: u.Quantity, shifts: u.Quantity, x_grid: np.ndarray, y_grid: np.ndarray, *, seeing: u.Quantity, zenith_angle: u.Quantity, beta: float, fwhm_func: Callable[[u.Quantity, u.Quantity, u.Quantity], u.Quantity]) -> np.ndarray:
    ...
    for shift, fwhm in zip(shift_values, fwhm_values, strict=True):
        for xpos, ypos, weight in zip(x, y, weights, strict=True):
            image += weight * _moffat_image(x_grid, y_grid, x0=xpos, y0=ypos + shift, fwhm=float(fwhm), beta=beta)
...
def _slit_throughput_curve(wave: u.Quantity, shifts: u.Quantity, *, seeing: u.Quantity, zenith_angle: u.Quantity, slit_width: u.Quantity, slit_length: u.Quantity, beta: float, grid_step: u.Quantity, fwhm_func: Callable[[u.Quantity, u.Quantity, u.Quantity], u.Quantity] | None = None) -> np.ndarray:
    ...
    slit_mask = (np.abs(x_grid) <= 0.5 * length) & (np.abs(y_grid) <= 0.5 * width)
    pixel_area = step**2

    throughput = np.empty(wave.size, dtype=float)
    for idx, (shift, fwhm) in enumerate(zip(shifts_arcsec, fwhm_values, strict=True)):
        image = _moffat_image(x_grid, y_grid, x0=0.0, y0=float(shift), fwhm=float(fwhm), beta=beta)
...
def _unique_float_values(values: u.Quantity, *, atol: float = 1.0e-9) -> list[float]:
    finite = sorted(float(value) for value in u.Quantity(values).to_value(u.arcsec) if np.isfinite(value))
    unique: list[float] = []
    for value in finite:
...
def _slit_loss_slit_specs(selector_widths: u.Quantity, current_slit: u.Quantity) -> OrderedDict[str, dict[str, Any]]:
    ...
    if not any(np.isclose(value, current, rtol=0.0, atol=1.0e-6) for value in values):
        values.append(current)
        values = sorted(values)

    if len(values) > 2:
        selected = [values[0], values[-1]]
        if not any(np.isclose(value, current, rtol=0.0, atol=1.0e-6) for value in selected):
    ...
    for value in sorted(selected):
        ...
        if len(supported) >= 2 and np.isclose(value, supported[0], rtol=0.0, atol=1.0e-6):
            role = "narrowest"
        elif len(supported) >= 2 and np.isclose(value, supported[-1], rtol=0.0, atol=1.0e-6):
            role = "widest"
        else:
            role = "current"
        key = "current" if current_match else f"{value:.3f}".rstrip("0").rstrip(".")
        if key in specs:
            continue
        specs[key] = {
            "label": f'{value:.2f}"',
...
def _slit_loss_airmass_specs(current_airmass: float) -> OrderedDict[str, dict[str, Any]]:
...
def build_slit_adc_psf_scene_data(
    ...
    wave = np.linspace(310, 980, 41) * u.nm if wave_nm is None else u.Quantity(wave_nm).to(u.nm)
    ...
    adc_shift = ad_shift - _atmospheric_refraction_shift(wave, zenith_angle + adc_error, cmds, wave_ref=wave_ref)
    ...
    source_tables = OrderedDict((name, source_position_table(source)) for name, source in sources.items())
    ...
    y_extent = max(3.5, width + 2 * max_shift + 6 * max_fwhm, float(np.ptp(all_y)) + 2 * max_shift + 4 * max_fwhm)
    ...
    for name, table in source_tables.items():
        status = slit_pair_status_table(table, slit_width=slit_width, slit_length=slit_length)
        images = OrderedDict((key, _scene_image(table, wave, variant["shift_arcsec"], x_grid, y_grid, seeing=seeing, zenith_angle=zenith_angle, beta=beta, fwhm_func=fwhm_func)) for key, variant in variants.items())
        scenarios[name] = {
            "positions": table,
            "status": status,
            "images": images,
    ...
    return {
        ...
        "notes": [
            ...
            (
                f"PSF FWHM comes from active ScopeSim {effect_name(psf_effect)}." if psf_effect is not None else ("PSF FWHM uses a fallback diagnostic seeing law because no " "active ScopeSim Moffat-like PSF effect was available.")
            ),
        ],
    }
...
def build_slit_loss_data(
    ...
    cmds = _cmds_from_train_or_cmds(train_or_cmds)
    arms = arms or OrderedDict({"VIS": (310 * u.nm, 980 * u.nm, "!INST.vis_curr_slit"), "NIR": (980 * u.nm, 2450 * u.nm, "!INST.nir_curr_slit"),})
    seeing = _required_cmd_quantity(cmds, "!OBS.seeing", u.arcsec)
    ...
    for arm_name, (wave_min, wave_max, slit_key) in arms.items():
        wave = np.linspace(u.Quantity(wave_min).to_value(u.nm), u.Quantity(wave_max).to_value(u.nm), n_wave) * u.nm
                ...
                for airmass_name, airmass_spec in airmass_specs.items():
                    ...
                    for adc_name, adc_spec in adc_specs.items():
                        if adc_spec["mode"] == "adc":
                            shifts = ad_shift - _atmospheric_refraction_shift(wave, zenith_angle + adc_error, cmds, wave_ref=wave_ref)
                        else:
                            shifts = ad_shift
                        throughput = _slit_throughput_curve(wave, shifts, seeing=seeing, zenith_angle=zenith_angle, slit_width=slit_spec["slit_width"], slit_length=slit_length, beta=psf_spec["beta"], grid_step=grid_step, fwhm_func=psf_spec["fwhm_func"])
                        key = f"{psf_name}_slit_{slit_name}_airmass_" f"{airmass_name}_{adc_name}"
                        curves[key] = {
                            "label": (
                                f"{psf_spec['label']}, "
        ...
        alias_airmass_keys = {
            "current": next(key for key, spec in airmass_specs.items() if spec["current"]),
            "zenith": next(key for key, spec in airmass_specs.items() if np.isclose(spec["airmass"], 1.0, rtol=0.0, atol=1.0e-6)),
            "z60": next(key for key, spec in airmass_specs.items() if np.isclose(spec["airmass"], 2.0, rtol=0.0, atol=1.0e-6)),
        }
    ...
    return {
        ...
        "active_psf_mode": next(name for name, spec in psf_modes.items() if spec["current"]),
        "psf_modes": psf_modes,
...
def build_slit_width_loss_data(
    ...
    """
    cmds = _cmds_from_train_or_cmds(train_or_cmds)
    slit_widths = (np.linspace(0.25, 2.0, 20) * u.arcsec if slit_widths is None else _quantity_with_default_unit(slit_widths, u.arcsec)).to(u.arcsec)
    ...
    for arm_name, (wave_values, slit_key) in arms.items():
        ...
        for psf_name, psf_spec in psf_modes.items():
            for wave_value in wave:
                ...
                for slit_width in slit_widths:
                    throughput = _slit_throughput_curve(one_wave, zero_shift, seeing=seeing, zenith_angle=zenith_angle, slit_width=slit_width, slit_length=slit_length, beta=psf_spec["beta"], grid_step=grid_step, fwhm_func=psf_spec["fwhm_func"])
        ...
        arm_data[arm_name] = {
            ...
            "selector_slit_widths_arcsec": _slit_selector_widths_for_key(train_or_cmds, slit_key),
            "wavelengths_nm": wave,
            "slit_length_arcsec": u.Quantity(slit_length).to(u.arcsec),
            "curves": curves,
        }
...
def validate_emissivity_sanity_data(data: Mapping[str, Any]) -> None:
    ...
    for aperture_id, channel in data["channels"].items():
        if not channel["pre_disperser_terms"]:
            raise ValueError(f"No pre-disperser emissivity terms for aperture_id={aperture_id}")
        if not channel["post_disperser_terms"]:
            raise ValueError(f"No post-disperser emissivity terms for aperture_id={aperture_id}")
        checks = [
        ...
        for label, arr in checks:
            ...
            if finite.size == 0:
                raise ValueError(f"Empty/non-finite {label} curve for aperture_id={aperture_id}")
            if np.nanmin(finite) < -1e-6:
                warnings.warn(f"Negative {label} curve for aperture_id={aperture_id}: " f"{np.nanmin(finite):.3g}", stacklevel=2)
...
def _resolved_meta_summary(effect: Any, cmds: Any) -> str:
    ...
    keys = ("filename", "dit", "ndit", "value", "noise_std", "bias", "binx", "biny", "current_slit", "fwhm", "alpha", "strehl", "wavelength")
    parts = []
    for key in keys:
...
def detector_background_budget_table(ztrain: Any, post_diffuse_data: Mapping[str, Any] | None = None, post_diffuse_consistency: Table | None = None) -> Table:
    ...
    for detector in detectors:
        ...
        exposure = _selected_detector_effect(ztrain, "exposure_integration_selector", detector)
        dark_current = _selected_detector_effect(ztrain, "dark_current_selector", detector)
        read_noise = _selected_detector_effect(ztrain, "readout_noise_selector", detector)
        ...
        diffuse_rate = _post_diffuse_rate_for_image_plane(ztrain, image_plane_id, post_diffuse_data=post_diffuse_data, post_diffuse_consistency=post_diffuse_consistency)
        dark_rate = _resolved_detector_meta(dark_current, "value", ztrain.cmds, detector_id)
        read_noise_single = _resolved_detector_meta(read_noise, "noise_std", ztrain.cmds, detector_id)
        read_ndit = _resolved_detector_meta(read_noise, "ndit", ztrain.cmds, detector_id)
        bias_level = _resolved_detector_meta(bias, "bias", ztrain.cmds, detector_id)

        diffuse_counts = diffuse_rate * exposure_time
        dark_counts = dark_rate * exposure_time
        additive_signal = diffuse_counts + dark_counts
        full_well = _required_detector_cmd_float(ztrain.cmds, "!DET.full_well", detector_id)
        ...
        total_noise = np.sqrt(diffuse_noise**2 + dark_noise**2 + read_noise_total**2)
...
def science_truth_crosscheck_table(ztrain: Any, *, post_diffuse_data: Mapping[str, Any] | None = None, detector_budget: Table | None = None, extraction_pixels: float = 1.0, spectral_resolution: float | None = None) -> Table:
    ...
    if detector_budget is None:
        detector_budget = detector_background_budget_table(ztrain, post_diffuse_data=post_diffuse_data)
    ...
    for budget in detector_budget:
        ...
        pixel_area = u.Quantity(channel_data.get("pixel_area", np.nan * u.arcsec**2)).to(u.arcsec**2)
        ...
        resolution_element = wavelength_mid / spectral_resolution if np.isfinite(wavelength_mid) and np.isfinite(spectral_resolution) and spectral_resolution > 0 else np.nan
        diffuse_rate_arcsec2 = diffuse_rate / pixel_area_value if np.isfinite(pixel_area_value) and pixel_area_value > 0 else np.nan
        ...
        rows.append({
            ...
            "current_slit_arcsec": _current_slit_arcsec(ztrain, str(budget["channel"])),
            ...
            "signal_fraction_of_full_well": float(budget["signal_fraction_of_full_well"]),
...
def _zenith_angle_from_airmass(airmass: float) -> u.Quantity:
    ...
    return np.degrees(np.arccos(np.clip(1.0 / airmass, 0.0, 1.0))) * u.deg


def _trace_dispersion_nm_per_pixel(trace: Any, image_plane: Any | None, wave_mid_nm: float) -> float:
    ...
    if not np.isfinite(span_nm) or span_nm <= 0:
        return np.nan
    step_nm = min(max(abs(wave_mid_nm) * 1.0e-5, span_nm * 1.0e-4, 1.0e-5), 0.45 * span_nm)
    wave_pair = np.array([
...
def resolution_element_snr_summary_table(snr_images: list[np.ndarray], footprint_table: Table, *, titles: list[str] | None = None) -> Table:
    ...
    footprint_by_channel = {str(row["channel"]): row for row in footprint_table}
    rows: list[dict[str, Any]] = []
    for title, snr in zip(titles, snr_images, strict=True):
        ...
        median_pixel_snr = float(np.nanmedian(positive)) if positive.size else np.nan
...
def readout_delta_summary_table(signal_hdul: Any, reference_hdul: Any, titles: list[str] | None = None) -> Table:
    ...
    if len(signal_readouts) != len(reference_readouts):
        raise ValueError("signal_hdul and reference_hdul contain different readout counts: " f"{len(signal_readouts)} != {len(reference_readouts)}")
    titles = titles or [f"detector {idx}" for idx in range(len(signal_readouts))]
    rows = []
    for idx, (title, signal, reference) in enumerate(zip(titles, signal_readouts, reference_readouts, strict=False)):
        try:
            delta = np.asarray(signal.data, dtype=float) - np.asarray(reference.data, dtype=float)
        except AttributeError:
            delta = np.asarray(signal[1].data, dtype=float) - np.asarray(reference[1].data, dtype=float)
...
def _post_diffuse_channels_by_image_plane(post_diffuse_data: Mapping[str, Any] | None) -> dict[int, Mapping[str, Any]]:
    if post_diffuse_data is None:
        return {}
    return {int(channel["image_plane_id"]): channel for channel in post_diffuse_data.get("channels", {}).values()}
...
def _current_slit_arcsec(ztrain: Any, channel: str) -> float:
    key = "!INST.vis_curr_slit" if channel.upper() in {"B", "G", "R"} else ("!INST.nir_curr_slit")
    return _required_cmd_float(ztrain.cmds, key)


def _science_truth_note(status: str) -> str:
    prefix = "Diffuse rate is integrated image-plane background; " "resolution_element_nm is an anchor, not a spectral bin for this term."
    if status == "saturated":
        return f"{prefix} Additive detector signal exceeds configured full well."
    if status == "near_saturation":
        return f"{prefix} Additive detector signal is near full well."
    return prefix


def _selected_detector_effect(ztrain: Any, selector_name: str, detector_row: Any) -> Any:
    selector = get_effect(ztrain, selector_name)
    selector_value = _selector_value_for_detector(selector, detector_row)
    if selector_value is None:
        raise ValueError(f"{selector_name!r} selector_key={selector.meta.get('selector_key')!r} " f"cannot be resolved for detector_id={int(detector_row['detector_id'])}.")
    try:
        return resolve_effect(selector, selector_value)
    except KeyError as exc:
        raise KeyError(f"{selector_name!r} has no entry for selector value " f"{selector_value!r} while resolving detector_id=" f"{int(detector_row['detector_id'])}.") from exc


def _resolved_detector_meta(effect: Any, key: str, cmds: Any, detector_id: int, *, default: float | None = None) -> float:
    from scopesim.utils import from_currsys

    if key not in getattr(effect, "meta", {}):
        if default is not None:
            return float(default)
        raise ValueError(f"{effect_name(effect)!r} has no required metadata key {key!r} " f"for detector_id={detector_id}.")
    ...
    if isinstance(value, Mapping):
        if detector_id in value:
            ...
            raise ValueError(f"{effect_name(effect)!r} metadata key {key!r} has no " f"detector_id={detector_id} entry.")
        value = from_currsys(value, cmds)
    return float(value)


def _required_detector_cmd_float(cmds: Any, key: str, detector_id: int) -> float:
    value = _required_cmd_value(cmds, key)
    ...
    if isinstance(value, Mapping):
        if detector_id in value:
            ...
            raise ValueError(f"Required ScopeSim command {key!r} has no " f"detector_id={detector_id} entry.")
        value = _resolved_for_display(value, cmds)
    elif isinstance(value, (list, tuple, np.ndarray)) and not hasattr(value, "unit"):
        if len(value) <= detector_id:
            raise ValueError(f"Required ScopeSim command {key!r} has length {len(value)} " f"and no detector_id={detector_id} entry.")
...
def _post_diffuse_rate_for_image_plane(ztrain: Any, image_plane_id: int, *, post_diffuse_data: Mapping[str, Any] | None = None, post_diffuse_consistency: Table | None = None) -> float:
    if post_diffuse_consistency is not None:
        matches = post_diffuse_consistency[np.asarray(post_diffuse_consistency["image_plane_id"], dtype=int) == image_plane_id]
        if len(matches):
            if "effect_included" in matches.colnames and not bool(matches[0]["effect_included"]):
                return 0.0
    ...
    selector = get_effect(ztrain, "post_echelle_diffuse_background_selector", active_only=False)
...
def dichroic_path_throughput(dichroic_tree: Any, aperture_id: int, wave: u.Quantity) -> tuple[np.ndarray, OrderedDict[str, np.ndarray]]:
    ...
    if len(rows) != 1:
        raise ValueError(f"Expected one dichroic row for {id_col}={aperture_id}; found " f"{len(rows)}")
...
def build_transmission_sanity_data(ztrain: Any, wave_nm: u.Quantity | None = None, component_metadata: Mapping[str, Mapping[str, Any]] | None = None, qe_selector_name: str | None = "detector_qe_selector", active_only: bool = True) -> dict[str, Any]:
    ...
    for aperture_id in aperture_ids:
        ...
        components = channel_optical_components(ztrain, aperture_id, qe_selector=qe_selector)
        surface_rows = optical_surface_rows(components, wave, component_metadata=component_metadata)
        optics_groups, group_counts = optical_surface_group_throughputs(surface_rows, wave)
        telescope_throughput = optics_groups.get("telescope", np.ones(wave.size, dtype=float))
        instrument_optics_groups = OrderedDict((name, values) for name, values in optics_groups.items() if name != "telescope")
        optics_total = np.prod(list(optics_groups.values()), axis=0) if optics_groups else np.ones(wave.size)
        instrument_optics_total = np.prod(list(instrument_optics_groups.values()), axis=0) if instrument_optics_groups else np.ones(wave.size)
        image_plane = None
        traces = traces_for_aperture.get(aperture_id, [])
        if traces:
            image_plane_id = int(traces[0].meta["image_plane_id"])
            if hasattr(ztrain, "image_planes") and image_plane_id < len(ztrain.image_planes):
                image_plane = ztrain.image_planes[image_plane_id]

        qe_values = effective_diffuse_qe(detector_qe, wave, footprint=image_plane) if getattr(detector_qe, "uses_detector_footprint", False) else evaluate_throughput(detector_qe, wave)
        qe_midpoint_values = evaluate_throughput(detector_qe, wave)
        pre_disperser_instrument_total = dichroic_total * instrument_optics_total
        ...
        for trace in traces:
            ...
            order_qe, qe_method = evaluate_trace_detector_qe(detector_qe, trace, wave, image_plane=image_plane)
        ...
        channels[aperture_id] = {
            ...
            "detector_qe_label": (
                "detector QE axis average" if getattr(detector_qe, "uses_detector_footprint", False) else "detector QE"
            ),
            "detector_qe_midpoint_label": (
                "detector QE at taper midpoint" if getattr(detector_qe, "uses_detector_footprint", False) else "detector QE midpoint"
            ),
            "detector_qe_accounting": detector_qe_accounting_summary(detector_qe),
...
def validate_transmission_sanity_data(data: Mapping[str, Any]) -> None:
    ...
    for aperture_id, channel in data["channels"].items():
        ...
        arrays.extend(order["total_with_telescope_no_slit"] for order in channel["orders"].values())
        for arr in arrays:
            finite = arr[np.isfinite(arr)]
            if finite.size == 0:
                raise ValueError(f"Empty/non-finite curve for aperture_id={aperture_id}")
            if np.nanmin(finite) < -1e-6 or np.nanmax(finite) > 1.5:
                warnings.warn(f"Throughput outside expected range for " f"aperture_id={aperture_id}: " f"{np.nanmin(finite):.3g}..{np.nanmax(finite):.3g}", stacklevel=2)


def snr_selected_spectral_bins(paired_samples: Table, *, bin_factors: tuple[int, ...], integration: str, slit_arcsec: float, ao_enabled: bool) -> tuple[Table, Table]:
    ...
    """
    native = paired_samples[(paired_samples["bin_factor"] == 1) & (paired_samples["native_count"] == 1)]
    unsorted_channels = np.asarray(native["channel"], dtype=str)
    channel_order = sorted(set(unsorted_channels), key=lambda channel: np.median(native["wave_nm"][unsorted_channels == channel]))
    native = native[np.lexsort((np.asarray(native["sample_index"], dtype=int), np.asarray(native["trace_id"], dtype=str), np.asarray(native["channel"], dtype=str)))]

    sky_models = ("simulation", "no OH")
    source_signal_native = np.stack((np.asarray(native["simulation_signal_e"], dtype=float), np.asarray(native["signal_e"], dtype=float)))
    source_shot_variance_native = np.stack((np.asarray(native["simulation_source_var_e"], dtype=float), np.asarray(native["source_var_e"], dtype=float)))
    non_source_variance_native = np.stack((np.asarray(native["simulation_fixed_variance_e"], dtype=float), np.asarray(native["fixed_variance_e"], dtype=float)))
    sky_variance_native = np.stack((np.asarray(native["simulation_background_var_e"], dtype=float), np.asarray(native["background_var_e"], dtype=float)))
    detector_variance_native = np.stack((np.asarray(native["simulation_detector_var_e"], dtype=float), np.asarray(native["detector_var_e"], dtype=float)))
    ...
    added_configured_sky_e = np.asarray(native["simulation_background_e"] - native["background_e"], dtype=float)
    ...
    for bin_factor in bin_factors:
        ...
        for channel in channel_order:
            ...
            trace_order = sorted(set(traces[channel_rows]), key=lambda value: (str(value).rpartition("_")[0], int(str(value).rpartition("_")[2])))
            for trace_id in trace_order:
                ...
                new_run = np.r_[True, (np.diff(native["sample_index"][indices]) != 1) | (native["x1"][indices[:-1]] != native["x0"][indices[1:]]) | ~np.isclose(native["wave_high_nm"][indices[:-1]], native["wave_low_nm"][indices[1:]], rtol=0, atol=1e-12),]
                ...
                for contiguous_order_run in np.unique(contiguous_runs):
                    ...
                    for bin_index_within_run, start in enumerate(range(0, len(run_indices), bin_factor)):
        ...
        assert np.all(np.bincount(padded_native_index[native_present], minlength=len(native)) == 1)
        ...
        def gather(values):
            ...
            if values.ndim == 1:
                values = values[None, :]
            return np.take_along_axis(values[:, None, :], safe_index[None, :, :], axis=2)
        ...
        resolution_width_nm = np.where(native_present, gather(native_resolution_width_nm)[0], 0.0).sum(axis=1)

        valid = native_present[None, :, :] & np.isfinite(source_signal) & np.isfinite(source_shot_variance) & np.isfinite(non_source_variance) & (source_shot_variance + non_source_variance > 0)
        available_count = valid.sum(axis=2)
        assert np.all(available_count > 0)
        native_snr = np.full_like(source_signal, -np.inf)
        native_snr[valid] = source_signal[valid] / np.sqrt(source_shot_variance[valid] + non_source_variance[valid])
        ...
        sorted_variance = np.take_along_axis(available_source_variance + available_non_source_variance, sort_order, axis=2)
        prefix_available = np.arange(bin_factor)[None, None, :] < available_count[:, :, None]
        cumulative_snr = np.full_like(sorted_signal, np.nan)
        cumulative_snr[prefix_available] = np.cumsum(sorted_signal, axis=2)[prefix_available] / np.sqrt(np.cumsum(sorted_variance, axis=2)[prefix_available])
        best_prefix_count = np.nanargmax(cumulative_snr, axis=2) + 1
        sorted_retained = np.arange(bin_factor)[None, None, :] < best_prefix_count[:, :, None]
        ...
        selected_sums = {name: (values * retained_mask).sum(axis=2) for name, values in components.items()}
        all_total_variance = all_sums["source_shot_variance_e2"] + all_sums["non_source_variance_e2"]
        selected_total_variance = selected_sums["source_shot_variance_e2"] + selected_sums["non_source_variance_e2"]
        all_snr = all_sums["source_signal_e"] / np.sqrt(all_total_variance)
        selected_snr = selected_sums["source_signal_e"] / np.sqrt(selected_total_variance)
        assert np.all(selected_snr >= all_snr - 1e-12 * np.maximum(1.0, np.abs(all_snr)))

        available_width_nm = np.where(valid, wavelength_width_nm[None, :, :], 0.0)
        selected_wavelength_fraction = (available_width_nm * retained_mask).sum(axis=2) / available_width_nm.sum(axis=2)
        all_effective_wave_nm = (wavelength_nm[None, :, :] * available_signal).sum(axis=2) / all_sums["source_signal_e"]
        selected_effective_wave_nm = (wavelength_nm[None, :, :] * available_signal * retained_mask).sum(axis=2) / selected_sums["source_signal_e"]
        selected_signal_fraction = selected_sums["source_signal_e"] / all_sums["source_signal_e"]
        ...
        for sky_index, sky_model in enumerate(sky_models):
            for selection_name, sums, total_variance, snr, counts, width_fraction, effective_wave, signal_fraction in (
                ...
                discriminant = (q2 * source_variance) ** 2 + 4 * signal**2 * q2 * non_source
                flux_scale = (q2 * source_variance + np.sqrt(discriminant)) / (2 * signal**2)
                limiting_magnitude = input_abmag - 2.5 * np.log10(flux_scale)

                for bin_index, metadata in enumerate(bin_metadata):
                    nominal_wave_nm = 0.5 * (metadata["nominal_wavelength_low_nm"] + metadata["nominal_wavelength_high_nm"])
                    complete = len(bin_native_indices[bin_index]) == bin_factor and available_count[sky_index, bin_index] == bin_factor
                    curve_rows.append({
                        ...
                        **{name: values[sky_index, bin_index] for name, values in sums.items()},
...
def fixed_bin_no_oh_limiting_magnitude_curves(samples: Table, *, bin_factors: tuple[int, ...], integration: str, slit_arcsec: float, ao_enabled: bool) -> Table:
    ...
    native = samples[(samples["bin_factor"] == 1) & (samples["native_count"] == 1)]
    channels = np.asarray(selected["channel"], dtype=str)
    traces = np.asarray(selected["trace_id"], dtype=str)
    channel_order = sorted(set(channels), key=lambda channel: np.median(selected["wave_nm"][channels == channel]))
    rows = []

    for channel in channel_order:
        channel_rows = channels == channel
        for trace_id in sorted(set(traces[channel_rows])):
            trace_native = native[(native["channel"] == channel) & (native["trace_id"] == trace_id)]
            trace_native = trace_native[np.argsort(trace_native["sample_index"])]
            native_sample_indices = np.asarray(trace_native["sample_index"], dtype=int)
            assert np.all(np.diff(native_sample_indices) > 0)
            native_signal_cumulative = np.r_[0.0, np.cumsum(np.asarray(trace_native["signal_e"], dtype=float))]
            native_wave_signal_cumulative = np.r_[0.0, np.cumsum(np.asarray(trace_native["wave_nm"], dtype=float) * np.asarray(trace_native["signal_e"], dtype=float)),]
            for bin_factor in bin_factors:
                indices = np.flatnonzero(channel_rows & (traces == trace_id) & (selected["bin_factor"] == bin_factor))
                indices = indices[np.argsort(selected["bin_index"][indices])]
                if len(indices) == 0:
                    continue
                new_run = np.r_[True, (np.diff(selected["bin_index"][indices]) != 1) | (selected["native_stop_index"][indices[:-1]] != selected["native_start_index"][indices[1:]]) | ~np.isclose(selected["wave_high_nm"][indices[:-1]], selected["wave_low_nm"][indices[1:]], rtol=0, atol=1e-12),]
                contiguous_runs = np.cumsum(new_run)
                for contiguous_order_run in np.unique(contiguous_runs):
                    run_indices = indices[contiguous_runs == contiguous_order_run]
                    for bin_index_within_run, row_index in enumerate(run_indices):
                        ...
                        native_start_position = np.searchsorted(native_sample_indices, native_start)
                        native_stop_position = np.searchsorted(native_sample_indices, native_stop)
                        assert np.array_equal(native_sample_indices[native_start_position:native_stop_position], np.arange(native_start, native_stop))
                        effective_wave_nm = (native_wave_signal_cumulative[native_stop_position] - native_wave_signal_cumulative[native_start_position]) / (native_signal_cumulative[native_stop_position] - native_signal_cumulative[native_start_position])
                        rows.append({
                            "integration": integration,
                            "slit_arcsec": float(slit_arcsec),
                            "ao_enabled": bool(ao_enabled),
                            ...
                            "total_variance_e2": float(sample["source_var_e"] + sample["fixed_variance_e"]),
