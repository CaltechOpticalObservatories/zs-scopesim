...
def read_lamp_lines(line_dir: str | pathlib.Path, filenames: Sequence[str] = DEFAULT_LAMP_LINE_FILENAMES) -> Table:
...
def lamp_flat(*, line_dir: str | pathlib.Path | None = None, zshooter_dir: str | pathlib.Path | None = None, filenames: Sequence[str] = DEFAULT_LAMP_LINE_FILENAMES, wave_step: u.Quantity = 0.05 * u.AA, resolving_power: float = 8e4, extent: float = 60):
    ...
    spectrum = source_templates.SourceSpectrum(source_templates.Empirical1D, points=waves, lookup_table=flux_density)
...
def constant_photon_flux_flat(amplitude: float = 0.001, extent: float = 60, wave_min: float = 100, wave_max: float = 300000, n_samples: int = 50000):
    ...
    spectrum = SourceSpectrum(Empirical1D, points=waves, lookup_table=spectrum_model(waves))
...
def two_point_source(*, separation: u.Quantity = 1.0 * u.arcsec, angle_on_slit: u.Quantity = 0.0 * u.deg, center: tuple[u.Quantity, u.Quantity] = (0.0 * u.arcsec, 0.0 * u.arcsec), centered: bool = True, spectrum: Any | None = None, mag: float = 20.0, weights: Sequence[float] = (1.0, 1.0)):
...
def field_angle_demo_sources(*, along_separation: u.Quantity = 5.0 * u.arcsec, across_separation: u.Quantity = 0.9 * u.arcsec, angle_on_slit: u.Quantity = 0.0 * u.deg, along_mag: float = 15.0, across_mag: float = 17.0, spectrum: Any | None = None) -> OrderedDict[str, Any]:
    ...
    scenarios = OrderedDict([
        (
            "along_slit_centered",
            two_point_source(separation=along_separation, angle_on_slit=angle_on_slit, centered=True, mag=along_mag, spectrum=spectrum)
        ),
        (
            "across_slit_one_off",
            two_point_source(separation=across_separation, angle_on_slit=angle_on_slit + 90 * u.deg, centered=False, mag=across_mag, spectrum=spectrum)
        ),
    ])
    descriptions = {
        "along_slit_centered": "Pair centered on the slit and separated along the slit.",
